...
export class AuthController {
  ...
  async getLogin(@Query() id, @Res() res, @Body() body) {
    ...
    return;
    res.redirect(
      'http://localhost:8080/authenticate?access_token=' + code.acess_token,
    );
  ...
  async authenticate2FA(@GetUser() jwtUser, @Res() res) {
    ...
    res.redirect(
      'http://localhost:8080/auth?access_token=' +
        (await (
          await this.authService.signJwtToken(parseInt(user.id), true)
        ).acess_token),
    );
  ...
  async turnOff2FA(@GetUser('sub') id) {
    ...
    if (user.secret2FA == null)
      throw new UnauthorizedException(
        "You don't have the two factor authentication enabled",
      );

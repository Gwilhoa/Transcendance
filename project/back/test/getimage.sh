...
contentType=$(curl -I -s http://localhost:6200/user/image -H "authorization: bearer $ACCESS_TOKEN" | grep 'Content-Type' | awk -F '/' '{print $2}' | tr -d '\r\n')

# Télécharger l'image en utilisant curl et écrire le contenu dans un fichier avec l'extension correspondante
curl -s http://localhost:6200/user/image -H "authorization: bearer $ACCESS_TOKEN" > test.${contentType}

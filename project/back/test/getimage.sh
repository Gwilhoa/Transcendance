...
contentType=$(curl -I -s http://localhost:3000/user/image -H "authorization: bearer $1" | grep 'Content-Type' | awk -F '/' '{print $2}' | tr -d '\r\n')

# Télécharger l'image en utilisant curl et écrire le contenu dans un fichier avec l'extension correspondante
curl -s http://localhost:3000/user/image -H "authorization: bearer $1" > test.${contentType}
